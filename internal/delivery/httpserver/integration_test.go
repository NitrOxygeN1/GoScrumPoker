...
func TestIntegration_Health(t *testing.T) {
	...
	if body.Database.Status != "ok" {
		t.Fatalf("database.status: got %q want ok", body.Database.Status)
	}
	if body.Database.Backend != "memory" {
		t.Fatalf("database.backend: got %q want memory", body.Database.Backend)
	}
}
